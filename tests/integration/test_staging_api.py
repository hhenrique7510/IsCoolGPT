...
class TestAskEndpoint:
    ...
    def test_ask_endpoint_exists(self, client):
        ...
        response = client.post(
            "/api/v1/ask", json={"question": "Teste"}, headers={"Content-Type": "application/json"}
        )
    ...
    def test_ask_endpoint_validation(self, client):
        ...
        response = client.post("/api/v1/ask", json={}, headers={"Content-Type": "application/json"})
        assert response.status_code == 422

    def test_ask_endpoint_empty_question(self, client):
        """Verifica se question vazio é rejeitado."""
        response = client.post(
            "/api/v1/ask", json={"question": ""}, headers={"Content-Type": "application/json"}
        )
        assert response.status_code == 422
